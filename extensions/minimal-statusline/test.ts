...
import minimalStatusline, { loadSettings, renderStatusline, saveSettings, summarizeUsage } from "./src/index.ts";
...
test("balanced footer includes native model, thinking, token, and cost information", () => {
  ...
  const line = renderStatusline(
    theme,
    ctx,
    { project: "my-pi-setup", nodeVersion: "v22.20.0", thinkingLevel: "medium", usage: summarizeUsage(ctx) },
...
test("minimal footer omits provider, Node, and token totals but preserves thinking", () => {
  ...
  const line = renderStatusline(
    theme,
    ctx,
    { project: "my-pi-setup", nodeVersion: "v22.20.0", thinkingLevel: "max", usage: summarizeUsage(ctx) },
    "branch",
    "minimal",
    200,
  );
...
test("footer prioritizes right-side Pi state within narrow terminal widths", () => {
  ...
  const line = renderStatusline(
    theme,
    ctx,
    { project: "a-very-long-project-name", nodeVersion: "v22.20.0", thinkingLevel: "high", usage: summarizeUsage(ctx) },
